...
export async function deleteTask(profile: Profile, taskId: string): Promise<void> {
  ...
  if (task.relatedWorkflowInstanceId) {
    // workflow_instance_steps.generated_task_id references tasks(id) with no ON DELETE
    // action, so deleting a workflow-linked task would otherwise raise a raw FK-violation
    // error that surfaces as a generic 500.
    throw new UnprocessableRequestError("Cannot delete a task generated by an active workflow");
  }
