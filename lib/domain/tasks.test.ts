...
describe.skipIf(!process.env.SUPABASE_SERVICE_ROLE_KEY)(
  ...
  () => {
    ...
    it("advances the linked workflow when a task generated by that workflow is completed", async () => {
      ...
      // The generated task's related_workflow_instance_id has no ON DELETE action
      // (see the migration adding that column), so the task that references this
      // instance must be removed before the instance itself, or the instance
      // delete below throws a foreign key violation.
